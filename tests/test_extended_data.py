...
class ExtendedDataTests(unittest.TestCase):
    ...
    def test_seasonal_feed_emits_transitions_instead_of_daily_labels(self) -> None:
        ...
        wuhou = next(item for item in winter_solstice if item.title == "蚯蚓结")
        self.assertEqual(
            "节气阶段：冬至 初候\n七十二候：蚯蚓结",
            wuhou.description,
        )
